...
public class TokyoTest extends BaseTests {
    ...
    public void tokyoTest() throws InterruptedException {
        ...
        String option = "Tokyo CURA Healthcare Center";
        makeAppointmentPage.selectFromDropDown("Tokyo CURA Healthcare Center");
        java.util.List<String> selectedOptions = makeAppointmentPage.getSelectedOptions();
        assertEquals(selectedOptions.size(),1,"Incorrect number of selections");
        assertTrue(selectedOptions.contains(option),"Option not selected");

        // Check the Box
        makeAppointmentPage.checkBox();
        TimeUnit.SECONDS.sleep(1);

        // Choose radio button
        makeAppointmentPage.buttonMedicare();
        TimeUnit.SECONDS.sleep(1);
